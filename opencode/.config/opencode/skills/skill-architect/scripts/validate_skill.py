    ...
    while i < len(lines):
        ...
        if line.startswith(" "):
            raise FrontmatterParseError(
                f"Unexpected indentation at top level near line {i + 1}: {line}"
            )
        ...
        if raw_value in block_markers:
            ...
            parsed_block, next_idx = _collect_block_scalar(
                lines, i + 1, min_indent=2, folded=folded
            )
        ...
        if not any(
            nl.strip() and not nl.strip().startswith("#") for nl in nested_lines
        ):
            data[key] = ""
            i = j
            continue
...
def validate_skill(skill_path: str) -> dict:
    ...
    add_check(
        "skill_md_exists",
        has_skill_md,
        "SKILL.md exists" if has_skill_md else "SKILL.md not found (case-sensitive)",
    )
    ...
    if wrong_casings:
        add_check(
            "skill_md_casing",
            False,
            f"Found wrong casing: {wrong_casings[0]} (must be exactly SKILL.md)",
        )
    ...
    add_check(
        ...
        if not has_readme
        else "README.md found — remove it (skills are for agents, not humans)",
    )
    ...
    if not fm_match:
        add_check(
            ...
            False,
            "Missing or malformed --- delimiters in frontmatter",
        )
    ...
    try:
        ...
        if parser_mode == "pyyaml":
            add_check(
                ...
                True,
                "Frontmatter is valid YAML (parsed with PyYAML)",
            )
        else:
            add_check(
                "frontmatter_valid_yaml",
    ...
    if not name:
        ...
        add_check(
            "name_kebab_case",
            is_name_kebab,
            f"name '{name}' {'is' if is_name_kebab else 'is NOT'} kebab-case",
        )
        ...
        add_check(
            "name_not_reserved",
            not has_reserved,
            "Name does not use reserved terms"
            if not has_reserved
            else "Name contains 'claude' or 'anthropic' (reserved)",
        )
        ...
        add_check(
            "name_matches_folder",
            names_match,
            f"name '{name}' matches folder '{folder_name}'"
            if names_match
            else f"name '{name}' does NOT match folder '{folder_name}'",
            severity="warning",
        )
    ...
    if not desc:
        add_check(
            "description_present", False, "Missing 'description' field in frontmatter"
        )
    else:
        desc_str = str(desc).strip()
        add_check(
            "description_present", True, f"description present ({len(desc_str)} chars)"
        )
        ...
        add_check(
            "description_length",
            desc_ok_length,
            f"Description length: {len(desc_str)}/1024 chars",
        )
        ...
        add_check(
            ...
            if not has_xml
            else "XML angle brackets found in description (forbidden)",
        )
        ...
        add_check(
            ...
            if has_triggers
            else "Missing trigger phrases — add 'Use when...' guidance (mandatory per CONTRIBUTING.md)",
        )
        ...
        add_check(
            ...
            else "Missing negative scope — add 'Do NOT use for...' guidance (mandatory per CONTRIBUTING.md)",
        )
    ...
    if not metadata or not isinstance(metadata, dict):
        ...
        add_check(
            "metadata_author",
            has_author,
            f"metadata.author: {meta_author}"
            if has_author
            else "Missing metadata.author",
            severity="warning",
        )
    ...
    add_check(
        ...
        if has_examples
        else "Consider adding usage examples",
        severity="warning",
    )

    # Check for error handling
    has_error_handling = bool(
        re.search(r"(?i)(error|fail|troubleshoot|issue|problem|if.*fails)", body)
    )
    add_check(
        "body_has_error_handling",
        has_error_handling,
        "Instructions include error handling"
        if has_error_handling
        else "Consider adding error handling guidance",
        severity="warning",
    )

    # --- Check 9: Optional files ---
    if "references" in entries and os.path.isdir(
        ...
        for ref in refs:
            ...
            add_check(
                f"ref_linked_{ref}",
                ref_mentioned,
                f"references/{ref} is referenced in SKILL.md"
                if ref_mentioned
                else f"references/{ref} exists but is not referenced in SKILL.md",
                severity="warning",
            )
...
def print_report(results: dict, verbose: bool = False):
    ...
    for check in results["checks"]:
        ...
        icon = (
            "✅"
            if check["passed"]
            else ("⚠️" if check["severity"] == "warning" else "❌")
        )
...
if __name__ == "__main__":
    ...
    if args.format in {"human", "both"}:
        ...
        if not args.json_out:
            print(
                "  Tip: add --json-out FILE to reuse this report without re-running.\n"
            )

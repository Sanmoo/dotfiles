...
function buildMarkdownContent(
	...
	const finalConversation =
		lastTurns && lastTurns > 0
			? sliceLastNTurns(conversation, lastTurns)
			: conversation;
	...
	if (branchInfo) {
		...
		lines.push(
			`leaf: ${branchInfo.leafId === null ? "null" : branchInfo.leafId}`,
		);
	}
	if (meta.sessionId) {
...
function describeToolFilter(toolFilter: ToolFilter | null): string | null {
	...
	const includes = [...toolFilter.includeNames]
		.sort()
		.map((name) => `+${name}`);
	const excludes = [...toolFilter.excludeNames]
		.sort()
		.map((name) => `-${name}`);
...
export default function (pi: ExtensionAPI) {
	pi.registerCommand("md", {
		...
		handler: async (args, ctx) => {
			...
			const tokens = argsTrimmed
				? argsTrimmed.split(/\s+/).filter(Boolean)
				: [];
			...
			if (tokensLower.includes("+all") || tokensLower.includes("-all")) {
				ctx.ui.notify(
					"'+all' and '-all' are not supported; use '/md tc' or '/md tc +tool'",
					"error",
				);
				return;
			}

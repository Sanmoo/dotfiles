...
const SYSTEM_PROMPT =
	...
	"Use up to 18 words in natural Title Case. Respond with only the title, no quotes or punctuation.";
...
async function openModelPicker(
	...
	if (!ctx.hasUI) {
		notify(
			ctx,
			"No interactive UI. Use: /rename model provider/model",
			"warning",
		);
		return null;
	}
	...
	return ctx.ui.custom<ModelRef | null>((tui, theme, _kb, done) => {
		...
		root.addChild(
			new Text(theme.fg("accent", theme.bold("Select Rename Model"))),
		);
		...
		const toItems = (q: string): SelectItem[] => {
			...
			return available
				.filter(
					(m) =>
						!lq ||
						formatRef(m).toLowerCase().includes(lq) ||
		...
		const rebuild = () => {
			...
			const idx = lastValue
				? items.findIndex((i) => i.value === lastValue)
				: -1;
...
async function generateName(
	...
	try {
		...
		if (response.stopReason === "error") {
			notify(
				ctx,
				`Rename failed: ${response.errorMessage ?? "unknown error"}`,
				"warning",
			);
			return null;
		}
...
export default function piAutoRename(pi: ExtensionAPI) {
	...
	async function autoName(ctx: ExtensionContext): Promise<void> {
		...
		try {
			const name = await generateName(
				ctx,
				modelRef,
				"Name this session based on the first user message. Create a descriptive one-line session title using up to 18 words.",
				`First user message:\n${firstMsg}`,
			);
	...
	pi.registerCommand("rename", {
		description:
			"Rename session with AI. Subcommands: model, show, reset, help.",
		...
		getArgumentCompletions: (prefix: string): AutocompleteItem[] | null => {
			...
			const refs = cachedModels
				.map(formatRef)
				.filter((r) => r.startsWith(rest));
			return refs.length > 0
				? refs.map((r) => ({ value: `model ${r}`, label: r }))
				: null;
		},
		...
		handler: async (args, ctx) => {
			...
			if (!trimmed) {
				...
				if (!transcript) {
					notify(
						ctx,
						"No conversation history to generate a name from.",
						"warning",
					);
					return;
				}
				const name = await generateName(
					ctx,
					modelRef,
					"Name this session based on the full conversation history. Create a descriptive one-line session title using up to 18 words.",
